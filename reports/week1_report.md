...
## 9. Addendum — Framework audit & lecturer dataset integration

After the lecturer issued the **PSA Framework** (a 4-step classification guide:
core intent → style/tone → audience → channel) and a pre-classified gold
dataset (`PSA_KE_Final.csv`), we audited our scraped collection against that
standard. This addendum supersedes the row counts above.

### What the audit found

Applying the framework strictly (a PSA must direct the public to act, avoid
something, or be alert), much of our scraped text did not qualify: WHO
fact-sheet definitions ("COVID-19 is the disease caused by…"), Infonet
encyclopedic farming descriptions, and press-release narrative about officials
and events. Sentence-splitting during scraping had also stripped action
context from many advisory paragraphs. A key calibration insight: the
lecturer's gold file itself applies a *broader* standard than the framework
text suggests — service announcements like "[KUCCPS Portal] KUCCPS portal will
open in March 2025…" are labelled PSA — so we calibrated our classifier
(documented, rule-based, in `SRC/psa_classify.py`) to his applied standard:
directive/advisory language, imperatives, audience markers and service/deadline
announcements keep a row; press-release, legal-notice, encyclopedic and
orphaned-continuation signals drop it.

### What we did (team decision)

- **Deleted 9,548 scraped rows** that failed the audit (of 10,365 scraped rows;
  817 high-confidence PSA rows kept). Deletion applies only to scraped rows;
  TICO-19 stays whole as corpus data (flagged in Metadata), as do our 150
  team-written pairs.
- **Merged the lecturer's gold dataset: 2,852 rows** (2,903 raw minus 8
  internal duplicates and cleaning losses), all pre-classified PSA, with
  **Ekegusii translations for 2,848 rows** — plus Dholuo and Somali, which we
  preserve in row Metadata. This single addition fixed our two biggest
  weaknesses at once: domain imbalance (it is Security/Education-heavy) and
  the empty Ekegusii column.
- Every row now carries a `psa_class` in its Metadata (PSA / PressRelease /
  Legal / Informational) so the classification decision is auditable per row.
  Full details: `reports/framework_audit.md`.

### Dataset after remediation (6,823 rows)

| Measure | Before audit | After remediation |
|---|---:|---:|
| Total rows | 13,519 | **6,823** |
| Paired EN–SW | 3,137 (23.2%) | **5,972 (87.5%)** |
| Paired with Ekegusii | 0 | **2,848 (41.7%)** |
| Composition | 17 scraped/corpus sources | Lecturer gold 2,852 · TICO-19 3,004 · scraped PSA 817 · team-written 150 |
